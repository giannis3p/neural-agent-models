...
#data split
train_size = int(0.72 * len(input_sequences))
...
def build_model(input_shape):
    ...
    x = layers.LSTM(100, return_sequences=True,kernel_regularizer=l2(0.01))(x)
    x = layers.LSTM(100)(x)
...
#compile
model.compile(optimizer='adam', loss='mse', metrics=[r_squared, 'mape', 'accuracy', 'msle', 'mae'])
...
df_all_timesteps.to_csv('CT-LSTM(100x100)(82-100hrs).csv', index=False)
...
csv_filename = 'ct_100_82_loss_data.csv'
...
plot_filename = f'{labels[i]}(t=82 to t=100)(time series plot)(100-ct-82).png'
plot_path = os.path.join(output_dir, plot_filename)
plt.savefig(plot_path)
plt.show()